...
static status_t ENET_MDIO_WriteExt(mdio_handle_t *handle, uint32_t phyAddr, uint32_t devAddr, uint32_t data)
{
    ...
    uint16_t devAddr2 = (uint16_t)((devAddr >> 16U) & 0x1FU);
	uint16_t regAddr = (uint16_t)(devAddr & 0xFFFFU);

    ENET_StartExtC45SMIWriteReg(base, phyAddr, devAddr2, regAddr);
    result = ENET_MDIO_WaitTransferOver(base);
    if (result != kStatus_Success)
    {
        return result;
    ...
    ENET_StartExtC45SMIWriteData(base, phyAddr, devAddr2, data);
    result = ENET_MDIO_WaitTransferOver(base);
    if (result != kStatus_Success)
    {
        return result;
